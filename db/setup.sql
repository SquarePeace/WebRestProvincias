WHILE EXISTS(SELECT NULL FROM sys.databases WHERE NAME='proof' OR NAME='proof')
BEGIN
    DECLARE @SQL VARCHAR(max)
    SELECT @SQL = COALESCE(@SQL,'') + 'Kill ' + Convert(VARCHAR, SPId) + ';'
    FROM MASTER..SysProcesses
    WHERE DBId = DB_ID(N'proof') OR DBId = DB_ID(N'proof') AND SPId <> @@SPId
    EXEC(@SQL)
    DROP DATABASE proof
    DROP DATABASE IF EXISTS proof
END
GO
